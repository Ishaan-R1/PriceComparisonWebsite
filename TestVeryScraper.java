...
@DisplayName("Test Very Scraper")
public class TestVeryScraper {
    static SessionFactory sessionFactory;
    static VeryScraper scraper;
    ...
    void testScraper() throws Exception {
        ...
        scraper = new VeryScraper();
        // Add DAO to very scraper
        scraper.setWebScrapeDao(dao);

        try {
            // Start thread to run very scraper
            scraper.start();
            Session session = sessionFactory.getCurrentSession();
            session.beginTransaction();
            // Declare an example of a phone model that would be pulled off very
            String model = "Galaxy S21 FE";
