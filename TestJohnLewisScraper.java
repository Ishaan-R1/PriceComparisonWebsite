...
@DisplayName("Test John Lewis Scraper")
public class TestJohnLewisScraper {
    static SessionFactory sessionFactory;
    static JohnLewisScraper scraper;
    ...
    void testScraper() throws Exception {
        ...
        scraper = new JohnLewisScraper();
        // Add DAO to john lewis scraper
        scraper.setWebScrapeDao(dao);

        try {
            // Start thread to run john lewis scraper
            scraper.start();
            Session session = sessionFactory.getCurrentSession();
            session.beginTransaction();
            // Declare an example of a phone model that would be pulled off john lewis
            String model = "Galaxy S22 5G Smartphone with Wireless PowerShare";
